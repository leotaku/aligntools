...
pub fn PerfectHash(comptime Target: type) type {
    return struct {
        E: []u8,
        m: usize,
        allocator: std.mem.Allocator,

        pub fn build(allocator: std.mem.Allocator, keys: []const []const u8) !@This() {
            if (keys.len > std.math.maxInt(Target)) {
                return error.PerfectHashOutOfRange;
            }
        ...
        pub fn hash(self: *@This(), key: []const u8) Target {
            var D = Hash.init(0);
            D.update(key);
...
test "no collisions" {
    ...
    var ph = try PerfectHash(u8).build(std.testing.allocator, &keys);
    defer ph.deinit();
